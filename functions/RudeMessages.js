...
module.exports = {
    ...
    /**
     * Sends a motivational quote when someone says something bad
    ...
    async execute(msg) {
        ...
        const H1 = [
            "My brother stop and listen to this:",
            "You can always be mad but just listen:",
            "Its okay we now you had a bad day but listen:",
            "Stop for a brief moment for this:",
        ];
        ...
        for (const m of message)
            if (
                m.match(
                    /^duha[sie]?$|^suk[aos]+$|^daun[asie]+$|^kekse$|^jiban[asie]+$|^pydar[asie]+$|^debila?s?i?e?(iukas)?$|^dalbajob[aesi]+$/gi
                )
            ) {
